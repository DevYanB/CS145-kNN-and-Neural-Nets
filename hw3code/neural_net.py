...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        a2 = np.maximum(a2, h1) # activation with input of h1
        h2 = np.dot(a2,W2.T) + b2
        scores = h2
        ...
        def MSE_loss(x, y):
            ...
            # x is our y_pred, and y is the y_target

            num_samples = x.shape[0]
            num_attrs = x.shape[1]
            y_target = np.zeros((num_samples, num_attrs))
            for i in range(num_samples):
              y_target[i][y[i]] = 1
            diff = x - y_target
            dx = diff / num_samples
            loss = 0.5 * np.sum(np.square(diff)) / num_samples
